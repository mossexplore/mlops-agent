...
from .database import list_knowledge_files, list_knowledge_revisions, upsert_knowledge_file
...
def write_markdown_knowledge(title: str, content: str, filename: Optional[str] = None) -> Dict[str, Any]:
    ensure_knowledge_dirs()
    ...
    record = record_markdown_file(path, create_revision=True)
    rebuild_index()
    return record
...
def retrieve_knowledge(query: str, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        return _retriever_module.retrieve_local_markdown_knowledge(query=query, top_k=top_k)
